...
int main(){
    ...
    int leadSpace = number2.find_first_not_of(' ');
    if (trailSpace == string::npos || leadSpace == string::npos){
        cout << "\nInvaild format, number can't spaces or empty!";
    }
    else {
        string number = number2.erase(trailSpace+1, len2-trailSpace);
        number = number2.erase(0, leadSpace);
